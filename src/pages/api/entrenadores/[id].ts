import { env } from 'cloudflare:workers';
import type { APIRoute } from 'astro';
import {
  getEntrenadorById,
  updateEntrenador,
  entrenadorSlugExists,
  parseOrdenDestacado,
  parseEloClasico,
  type Actividad,
} from '../../../lib/entrenadores';
import { slugify } from '../../../lib/text';
import { uploadCoverImage, deleteImage } from '../../../lib/images';

export const prerender = false;

export const POST: APIRoute = async ({ request, params, redirect }) => {
  const id = Number(params.id);
  if (!Number.isInteger(id) || id <= 0) {
    return redirect('/admin/entrenadores?error=' + encodeURIComponent('Identificador de entrenador inválido.'));
  }

  const existing = await getEntrenadorById(env.DB, id);
  if (!existing) {
    return redirect('/admin/entrenadores?error=' + encodeURIComponent('Entrenador no encontrado.'));
  }

  const form = await request.formData();
  const nombreCompleto = String(form.get('nombre_completo') ?? '').trim();
  const estadoRepublica = String(form.get('estado_republica') ?? '').trim() || null;
  const tituloAjedrez = String(form.get('titulo_ajedrez') ?? '').trim() || null;
  const tituloArbitraje = String(form.get('titulo_arbitraje') ?? '').trim() || null;
  const eloClasico = parseEloClasico(form.get('elo_clasico'));
  const fideId = String(form.get('fide_id') ?? '').trim() || null;
  const email = String(form.get('email') ?? '').trim() || null;
  const bio = String(form.get('bio') ?? '').trim() || null;
  const actividad = (String(form.get('actividad') ?? 'activo').trim() as Actividad) === 'inactivo' ? 'inactivo' : 'activo';
  const reclamado = form.get('reclamado') === 'on';
  const ordenDestacado = parseOrdenDestacado(form.get('orden_destacado'));
  const fotoFile = form.get('foto') as File | null;
  const removeFoto = form.get('remove_foto') === 'on';

  if (!nombreCompleto) {
    return redirect(`/admin/entrenadores/editar/${id}?error=` + encodeURIComponent('El nombre completo es obligatorio.'));
  }

  if (form.get('elo_clasico') && eloClasico === null) {
    return redirect(`/admin/entrenadores/editar/${id}?error=` + encodeURIComponent('El ELO clásico debe ser un número entero entre 0 y 3500.'));
  }

  let slug = existing.slug;
  const newSlug = slugify(nombreCompleto);
  if (newSlug !== existing.slug && !(await entrenadorSlugExists(env.DB, newSlug, id))) {
    slug = newSlug;
  }

  let fotoKey = existing.foto_key;
  if (fotoFile && fotoFile.size > 0) {
    try {
      const uploaded = await uploadCoverImage(env.R2_IMAGES, fotoFile, 'entrenadores');
      await deleteImage(env.R2_IMAGES, existing.foto_key);
      fotoKey = uploaded;
    } catch (err) {
      const msg = err instanceof Error ? err.message : 'No se pudo subir la foto.';
      return redirect(`/admin/entrenadores/editar/${id}?error=` + encodeURIComponent(msg));
    }
  } else if (removeFoto && existing.foto_key) {
    await deleteImage(env.R2_IMAGES, existing.foto_key);
    fotoKey = null;
  }

  const reclamadoEn = reclamado ? existing.reclamado_en ?? new Date().toISOString() : null;

  await updateEntrenador(env.DB, id, {
    slug,
    nombre_completo: nombreCompleto,
    estado_republica: estadoRepublica,
    titulo_ajedrez: tituloAjedrez,
    titulo_arbitraje: tituloArbitraje,
    elo_clasico: eloClasico,
    fide_id: fideId,
    foto_key: fotoKey,
    email,
    bio,
    actividad,
    reclamado_en: reclamadoEn,
    orden_destacado: ordenDestacado,
  });

  return redirect(`/admin/entrenadores?updated=${id}`);
};
